...
import { generate180DayRoadmap } from './server-backend/roadmap';
import { PlannerService } from './server-backend/planner';
import { MentorService } from './server-backend/mentor';
import { ReportsService } from './server-backend/reports';
...
export function createExpressApp() {
  ...
  app.post('/api/register', async (req: Request, res: Response) => {
    try {
      ...
      const isPermissionDenied = error?.message?.includes('permission-denied') || error?.message?.includes('permissions');
      res.status(500).json({ 
        error: isPermissionDenied 
          ? 'Database connection or permissions error. Please contact administrator or check database rules.' 
